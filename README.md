...
| `terraform-validate.yml` | — | `terraform-version` (`1.9.8`), `working-directory` (`.`), `fmt-check-recursive` (`true`), `tflint-version` (`v0.53.0`), `checkov-soft-fail` (`false`), `checkov-skip-checks` (`""`), `trivy-config-severity` (`HIGH,CRITICAL`), `run-gitleaks` (`true`), `run-terraform-docs-check` (`true`), `terraform-docs-working-dir` (`.`), `run-plan` (`false`), `plan-role-to-assume` (`""`), `aws-region` (`""`), `plan-args` (`""`), `runs-on` | — | `plan-exitcode` | jobs `validate`/`lint`/`security`/`docs`: `contents: read`; job `plan`: `+ id-token: write` | `echo-pong-gh-tf-plan` (only when `run-plan: true`) |
